...
class maxConnect4Game:
	...
	def utility(self,a,turn,function):
		tempObject=maxConnect4Game() #created an object for temporary purpose
		tempObject.gameBoard=deepcopy(a)
		tempObject.countScore()
		if function=='min':
			if turn ==1:
				return (tempObject.player2Score-tempObject.player1Score)
			else:
				return (tempObject.player1Score-tempObject.player2Score)
		else:
			if turn==1:
				return (tempObject.player1Score-tempObject.player2Score)
			else:
	...
	def miniMax(self):
		...
		choice=0
		for i in range(7):
			board,status=self.result(deepcopy(self.gameBoard),i,turn)
			if status=='yes':
				val=self.minValue(deepcopy(board),depth,negTurn)
				if(val>v):
					choice=i
					v=val
		print('choice selected %d:'%choice)
		return choice

	def minValue(self,a,depth,turn):
		print('minValue called')
		if(depth==0):
			print('------------------------------------Depth reached in minValue')
			return self.utility(a,turn,'min')
			negTurn=0
		if(turn==1):
			negTurn=2
		else:
		...
		for i in range(7):
			board,status=self.result(a,i,turn)
			if status=='no':
				return self.utility(a,turn,'min')
			else:
				return min(self.maxValue(deepcopy(board),depth-1,negTurn) for i in range(7))
		
	def maxValue(self,a,depth,turn):
		print('maxValue called')
		if(depth==0):
			print('-----------------------------------Depth reached in maxValue')
			return self.utility(a,turn,'max')
		negTurn=0
		if(turn==1):
			negTurn=2
		else:
		...
		for i in range(7):
			...
			if status=='no':
				...
				return max(self.minValue(deepcopy(board),depth-1,negTurn) for i in range(7))		

	# The AI section. Currently plays randomly.
	def aiPlay(self):
		randColumn = self.miniMax()
		result = self.playPiece(randColumn)
		if not result:
			print('calling aiplay again')
			self.aiPlay()
		else:
