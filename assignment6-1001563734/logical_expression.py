...
def check_true_false(knowledge_base, statement,model):
    ...
    reslt1 = tt_check_all(knowledge_base,statement,symbols,model1)
    ...
    model2 = model
    symbols = getSymbols(knowledge_base,symbols)
    reslt2 = tt_check_all(knowledge_base,notstatement,symbols,model2)
    try:
	f = open('result.txt','w')
    except:
...
def checkPLTrue(kb,model):
	exp = '('
	if re.match(r'^[M|P|S|B]_[1-4]_[1-4]$',kb.symbol[0]):
	    return str(model[kb.symbol[0]])
	elif kb.connective[0] == 'and':
	    exp = '(True and '
